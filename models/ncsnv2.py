...
class NCSN(nn.Module):
    ...
    def forward(self, x, y):
        ...
        layer2 = self._compute_cond_module(self.res2, output, y)
        layer3 = self._compute_cond_module(self.res3, output, y)
        layer4 = self._compute_cond_module(self.res4, output, y)

        ref1 = self.refine1([layer4], y, layer4.shape[2:])
        ref2 = self.refine1([layer4, ref1], y, layer3.shape[2:])
        ref3 = self.refine1([layer4, ref2], y, layer2.shape[2:])
...
class NCSNv2_256(nn.Module):
    ...
    def __init__(self, config):
        ...
        self.refine2 = RefineBlock([4 * self.nf, 4 * self.nf], 2 * self.nf, act=act)
        self.refine3 = RefineBlock([2 * self.nf, 2 * self.nf], 2 * self.nf, act=act)
        self.refine4 = RefineBlock([2 * self.nf, 2 * self.nf], 2 * self.nf, act=act)
        self.refine5 = RefineBlock([2 * self.nf, 2 * self.nf], self.nf, act=act)
        self.refine6 = RefineBlock([self.nf, self.nf], self.nf, act=act, end=True)
    ...
    def forward(self, x, y):
        ...
        layer4 = self._compute_cond_module(self.res4, layer3)
        layer5 = self._compute_cond_module(self.res5, layer4)
        layer6 = self._compute_cond_module(self.res6, layer5)

        ref1 = self.refine1([layer6], layer6.shape[2:])
        ref2 = self.refine2([layer5, ref1], layer5.shape[2:])
        ref3 = self.refine3([layer4, ref2], layer4.shape[2:])
        ref4 = self.refine4([layer3, ref3], layer3.shape[2:])
        ref5 = self.refine5([layer2, ref4], layer2.shape[2:])
        output = self.refine6([layer1, ref5], layer1.shape[2:])
